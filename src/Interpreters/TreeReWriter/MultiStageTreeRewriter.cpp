...
#if 0
using RewriteResult = MultiStageTreeReWriter::RewriteResult;
using RewriteResultPtr = MultiStageTreeReWriter::RewriteResultPtr;

RewriteResultPtr MultiStageTreeReWriter::analyze(ASTPtr query)
{
    RewriteResultPtr result = std::make_unique<RewriteResult>();
    if (!query->as<ASTSelectQuery>() && !query->as<ASTSelectWithUnionQuery>())
    {
        result->final_ast = query->clone();
        return result;
 
    }
    ASTSelectQuery * select_ast = query->as<ASTSelectQuery>();
    if (auto * union_select_ast = query->as<ASTSelectWithUnionQuery>())
    {
        if (union_select_ast->list_of_selects->children.size() != 1)
        {
            result->final_ast = query->clone();
            return result;
        }
        ...
        {
            select_ast = union_select_ast->list_of_selects->children[0]->as<ASTSelectQuery>();
        }
    }
    const auto * join_element = select_ast->join();
    if (!join_element)
    {
        LOG_TRACE(&Poco::Logger::get("MultiStageTreeRewriter"), "no join clause");
        result->final_ast = query->clone();
        return result;
    }
    else
    {
        auto ast = join_element->clone();
        LOG_TRACE(&Poco::Logger::get("MultiStagteTreeRewriter"), "join ast:{}", queryToString(ast));
    }
    
    JoinedTables joined_tables(getContext(), *select_ast, false);
    NamesAndTypesList source_columns;
    if (StoragePtr storage = joined_tables.getLeftTableStorage())
    {
        source_columns = storage->getInMemoryMetadataPtr()->getColumns().getAll();
    }
    else if (auto interpreter = joined_tables.makeLeftTableSubquery(SelectQueryOptions{}))
    {
        source_columns = interpreter->getSampleBlock().getNamesAndTypesList();   
    }
    auto table_expressions = getTableExpressions(*select_ast);
    for (auto & expression : table_expressions)
    {
        LOG_TRACE(logger, "table expression:{}", queryToString(*expression));
    }
    auto tables_with_columns = getDatabaseAndTablesWithColumns(table_expressions, getContext(), true, true);
    auto source_name_set = removeDuplicateColumns(source_columns);
    TranslateQualifiedNamesVisitor::Data qualified_data(source_name_set, tables_with_columns);
    DebugASTLog<false> log;
    auto select_query = select_ast->clone();
    TranslateQualifiedNamesVisitor(qualified_data, log.stream()).visit(select_query);


    

    ASTPtr tmp;
    getTablesHeader(*(select_query->as<ASTSelectQuery>()), tmp, tmp);
    result->final_ast = select_ast->clone();
    return result;
}

bool MultiStageTreeReWriter::getTablesHeader(const ASTSelectQuery & select_query_, ASTPtr & /*left_select_*/, ASTPtr & )
{
    auto table_expressions = getTableExpressions(select_query_);
    for (auto & expression : table_expressions)
    {
        LOG_TRACE(logger, "table expression:{}", queryToString(*expression));
    }
    auto tables_with_columns = getDatabaseAndTablesWithColumns(table_expressions, getContext(), true, true);
    for (auto & table_col : tables_with_columns)
    {
        LOG_TRACE(logger, "cols:{}", table_col.columns.toString());
    }
    LOG_TRACE(logger, "query select expr:{}", queryToString(select_query_.select()));
    for (auto & child : select_query_.select()->children)
    {
        LOG_TRACE(logger, "select child: {}", queryToString(child));
    }
    Aliases aliases;
    auto visit_ast = select_query_.clone();
    QueryAliasesVisitor(aliases).visit(visit_ast);
    LOG_TRACE(logger, "aliases size : {}", aliases.size());
    for(const auto & alias : aliases)
    {
        LOG_TRACE(logger, "key: {}, ast : {}", alias.first, queryToString(alias.second));
    }
    ASTs left_idents, right_idents;
    /*
    CollectJoinColumnsMatcher::Data collect_data{
        tables_with_columns[0],
        tables_with_columns[1],
        left_idents,
        right_idents,
    };
    LOG_TRACE(logger, "collect left/right table columns:{}", queryToString(select_query_.select()));
    CollectJoinColumnsVisitor(collect_data).visit(select_query_.select());
    */
    for (auto & ident : left_idents)
    {
        LOG_TRACE(logger, "left table ident from select expr: {}", queryToString(ident));
    }
    for (auto & ident : right_idents)
    {
        LOG_TRACE(logger, "right table ident from select expr: {}", queryToString(ident));
    }
    return false;
}

bool MultiStageTreeReWriter::getTablesHashKeys(ASTPtr /*select_query_*/)
{
    return false;
}
#endif
void DistributedJoinRewriteMatcher::visit(ASTPtr & ast_, Data & data_)
{
    ASTPtr final_ast;
    ...
    LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriteMatcher"), "stage query:{}", queryToString(data_.rewritten_query));

}

DistributedJoinRewriter::DistributedJoinRewriter(std::shared_ptr<ASTSelectQuery> query_, ContextPtr context_, size_t assigned_id_)
...
ASTPtr DistributedJoinRewriter::rewrite()
{
    if (!checkRewritable())
        return nullptr;
    ...
    if (!collectHashKeysList())
        return nullptr;
    //auto stages_query = std::make_shared<ASTStagedSelectQuery>();
...
bool DistributedJoinRewriter::prepare()
{
    if (!checkRewritable())
        return false;
    if (!collectTablesColumns())
...
bool DistributedJoinRewriter::checkRewritable()
{
    ...
    if (table_join_ast->on_expression)
    {
        ...
        {
            LOG_INFO(&Poco::Logger::get("DistributedJoinRewriter"), "Not support or join. {}", queryToString(*table_join_ast));
            return false;
        }
    }
    else {
        LOG_INFO(&Poco::Logger::get("DistributedJoinRewriter"), "Not support join format: {}", queryToString(*table_join_ast));
        return false;
    }
    return true;
}

bool DistributedJoinRewriter::collectTablesColumns()
{
    ...
    LOG_TRACE(
        &Poco::Logger::get("DistributedJoinRewriter"),
        "tables columns. left: {} \n right:{}",
...
bool DistributedJoinRewriter::collectTablesColumnsFromSelect()
{
    ...
    LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriter"), "left select cols: {}, right select cols:{}", tables_columns_from_select[0].toString(), tables_columns_from_select[1].toString());
    return true;
}

bool DistributedJoinRewriter::collectTablesColumnsFromOnJoin()
{
    auto * table_join_ast = joined_elements->table_join->as<ASTTableJoin>();
    LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriter"), "table join ast : {}", queryToString(*table_join_ast));
    if (table_join_ast->on_expression)
    {
...
bool DistributedJoinRewriter::collectHashKeysList()
{
    auto * table_join_ast = joined_elements->table_join->as<ASTTableJoin>();
    if (table_join_ast->on_expression)
    {
        ...
        else if (on_func->name == "and")
        {
            ...
            LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriter"), "Unsupported join function: {}", on_func->name);
            return false;
        }
    }
    else {
...
bool DistributedJoinRewriter::collectHashKeysListOnEqual(ASTPtr ast, ASTs & keys_list)
{
    ...
    ASTPtr left_key = nullptr, right_key = nullptr;
    {
        ...
        else if (columns[0].getNames().empty() && !columns[1].getNames().empty())
        {
            ...
            LOG_INFO(&Poco::Logger::get("DistributedJoinRewriter"), "Cannot find pos for arg: {}", queryToString(left_arg));
            return false;
        }
    }
    {
        ...
        else if (columns[0].getNames().empty() && !columns[1].getNames().empty())
        {
            ...
            LOG_INFO(&Poco::Logger::get("DistributedJoinRewriter"), "Cannot find pos for arg: {}", queryToString(right_arg));
            return false;
        }
    }
    if (!left_key || !right_key)
    {
        LOG_INFO(&Poco::Logger::get("DistributedJoinRewriter"), "Invalid join condition: {}", queryToString(ast));
        return false;
    }
...
bool DistributedJoinRewriter::collectHashKeysListOnAnd(ASTPtr ast, ASTs & keys_list)
{
    auto * func = ast->as<ASTFunction>();
    LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriter"), "fun args:{}", queryToString(func->arguments));
...
ASTPtr DistributedJoinRewriter::createNewJoinSelectQuery(const Strings & left_table_id, const Strings & right_table_id)
{
    ...
    LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriter"), "original left table expression: {}, alias:{}", queryToString(*getTableExpression(*query, 0)), tryGetTableExpressionAlias(getTableExpression(*query, 0)));
    LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriter"), "new join select query: {}", queryToString(select_query));
    return select_query;

}

ASTPtr DistributedJoinRewriter::createNewJoinSelectQuery(ASTPtr & left_query, ASTPtr & right_query)
{
    auto select_query = std::dynamic_pointer_cast<ASTSelectQuery>(query->clone());
    select_query->setExpression(ASTSelectQuery::Expression::TABLES, std::make_shared<ASTTablesInSelectQuery>());
    auto tables = select_query->tables();
...
ASTPtr DistributedJoinRewriter::createLeftTableQuery(const String & hash_table_id)
{
    ...
    LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriter"), "new left insert  query: {}", queryToString(insert_query));

    return insert_query;
}

ASTPtr DistributedJoinRewriter::createRightTableQuery(const String & hash_table_id)
{
    auto select_query = std::make_shared<ASTSelectQuery>();
    ...
    insert_query->table_function = hash_table;
    insert_query->select = select_with_union_query;
    LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriter"), "new right insert  query: {}", queryToString(insert_query));

    return insert_query;
}

ASTPtr DistributedJoinRewriter::createHashTableExpression(const String & table_id, NamesAndTypesList & columns, ASTPtr & hash_keys)
{
    auto table_func = std::make_shared<ASTFunction>();
    table_func->name = "HashTable";
...
ASTPtr DistributedJoinRewriter::createHashTableExpression(const Strings & table_id, NamesAndTypesList & columns, ASTPtr & hash_keys)
{
    auto table_func = std::make_shared<ASTFunction>();
    table_func->name = "HashTable";
    table_func->arguments = std::make_shared<ASTExpressionList>();
...
std::shared_ptr<ASTInsertQuery> DistributedJoinRewriter::createSubJoinTable(const Strings & table_id, std::shared_ptr<ASTTableExpression> table_expression)
{
    auto select_query = std::make_shared<ASTSelectQuery>();
...
ASTPtr DistributedJoinRewriteMatcher::visit(std::shared_ptr<ASTSelectQuery> & query_, Data & data_)
{
...
ASTPtr DistributedJoinRewriteMatcher::visit(std::shared_ptr<ASTSelectWithUnionQuery> & query_, Data & data_)
{
    ASTs prev_children;
...
ASTPtr DistributedJoinRewriteMatcher::visitChild(ASTPtr & query_, Data & data_)
{
    ASTPtr new_query;
    ...
    if (!new_query)
    {
        ...
        LOG_TRACE(&Poco::Logger::get("DistributedJoinRewriteMatcher"), "Rewrite ast failed. {}", queryToString(query_));
    }
    return new_query;

}

ASTPtr DistributedJoinRewriteMatcher::visitSelectWithJoin(std::shared_ptr<ASTSelectQuery> & query_, Data & data_)
{
    DistributedJoinRewriter rewriter(query_, data_.context, data_.id_count);
    if (!rewriter.prepare())
        return query_;
    ...
    auto staged_query = std::make_shared<ASTStagedSelectQuery>();
    staged_query->staged_insert_queries.emplace_back(left_insert_query);
    staged_query->staged_insert_queries.emplace_back(right_insert_query);
    staged_query->final_select_query = new_query;

    return staged_query;
}

std::shared_ptr<ASTTableExpression> DistributedJoinRewriteMatcher::visitJoinSelectTableExpression(const ASTTableExpression * table_expression, Data & data)
{
