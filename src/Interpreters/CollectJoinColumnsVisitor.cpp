...
#if 0
    const ASTIdentifier * unroll_ident = unrollAliases(&ident_, data_.aliases);
    const auto & name = unroll_ident->name();
    if (auto opt = IdentifierSemantic::getMembership(ident_); opt.has_value())
    {
        if (*opt == 0)
        {
            for (auto & col : tables[0])
            data_.left_columns.push_back(ident_.clone());
        }
        else if (*opt == 1)
        {
            data_.right_columns.push_back(ident_.clone());   
        }
        else
        {
            throw Exception(ErrorCodes::AMBIGUOUS_COLUMN_NAME, "Position of identifier {} can't be deteminated.", name);
        }
        return;
    }
    
    bool in_left_table = data_.left_table.hasColumn(name);
    bool in_right_table = data_.right_table.hasColumn(name);

    if (in_left_table && in_right_table)
    {
        if (auto original_name = IdentifierSemantic::uncover(*unroll_ident))
        {
            auto match = IdentifierSemantic::canReferColumnToTable(*original_name, data_.right_table.table);
        ...
        {
            throw Exception(ErrorCodes::AMBIGUOUS_COLUMN_NAME, "Ambiguous columns : {}. original ident: {}", name, queryToString(ident_));
        }
    }
    
    if (in_left_table)
    {
        data_.left_idents.push_back(ast_->clone());
    }
    else if (in_right_table)
    {
        data_.right_idents.push_back(ast_->clone());
    }
    else
    {
        LOG_TRACE(&Poco::Logger::get("CollectJoinColumnsVisitor"), "not found table for ident:{}", queryToString(ast_));
    }
#endif
}
