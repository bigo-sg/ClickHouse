...
HashJoin::HashJoin(std::shared_ptr<TableJoin> table_join_, const Block & right_sample_block_, bool any_take_last_row_, size_t reserve_size)
...
void HashJoin::dataMapInit(MapsVariant & map, size_t reserve_size)
{
    ...
    if (reserve_size)
    {
        joinDispatch(kind, strictness, map, [&](auto, auto, auto & map_) {map_.reserve(data->type, reserve_size); });
    }
}
