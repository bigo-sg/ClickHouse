...
DistributedShuffleJoinShufflePhase::DistributedShuffleJoinShufflePhase(std::shared_ptr<std::vector<std::shared_ptr<BlockIO>>> insert_block_ios_)
...
void DistributedShuffleJoinShufflePhase::runOnce()
{
...
DistributedShuffleJoinShuffleTransform::DistributedShuffleJoinShuffleTransform(std::shared_ptr<BlockIO> insert_block_io_)
    : ISource(Block{})
...
Chunk DistributedShuffleJoinShuffleTransform::generate()
{
    if (insert_finished)
        return {};
...
DistributedShuffleJoinTransform::DistributedShuffleJoinTransform(const Block & header_)
    : IProcessor({header_, Block{}}, {header_})
{

}

IProcessor::Status DistributedShuffleJoinTransform::prepare()
{
    static int count = 0;
...
void DistributedShuffleJoinTransform::work()
{
    if (has_input)
    {
        has_input = false;
