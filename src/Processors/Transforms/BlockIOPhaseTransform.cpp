...
SourceBlockIOPhaseTransform::SourceBlockIOPhaseTransform(const QueryBlockIO & block_io_)
    : ISource(block_io_.block_io->pipeline.getHeader())
...
BlockIOPhaseTransform::BlockIOPhaseTransform(const QueryBlockIO & block_io_, const Block & input_header_)
    : IProcessor({input_header_}, {block_io_.block_io->pipeline.getHeader()})
    , block_io(block_io_.block_io)
    , query(block_io_.query)
{
