...
public:
    struct Params
    {
        explicit Params(const Aggregator::Params & params_, bool final_)
            : aggregator_params(params_)
            , final(final_)
        {
            aggregator = std::make_unique<Aggregator>(aggregator_params);
        }
    ...
    explicit HighCardinalityAggregatingTransform(const Block & header, std::unique_ptr<Params> params_);
