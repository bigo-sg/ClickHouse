...
static InputPorts buildShuffleInputport(size_t num_streams, const Block & header)
{
    InputPorts inputs;
    for (size_t i = 0; i < num_streams; ++i)
    {
        InputPort input(header);
        inputs.push_back(input);
    }
    return inputs;
}

InnerShuffleCollectTransform::InnerShuffleCollectTransform(size_t num_streams_, const Block & header_)
    : IProcessor(buildShuffleInputport(num_streams_, header_), {header_})
    , header(header_)
{
}

IProcessor::Status InnerShuffleCollectTransform::prepare()
{
...
void InnerShuffleCollectTransform::work()
{
    if (has_input)
    {
        Blocks blocks;
        ...
        auto final_block = concatenateBlocks(blocks);
        input_chunks.clear();
        output_chunk = Chunk(final_block.getColumns(), final_block.rows());
        has_output = true;
        has_input = false;
    }
}
