...
namespace
{
    ...
    {
        auto builder = HDFSHandlerFactory::instance().getBuilder(uri_without_path + "/", context->getGlobalContext()->getConfigRef());
...
public:
    explicit Impl(const std::vector<const String> & uris_, ContextPtr context)
    {
        ...
        auto builder = HDFSHandlerFactory::instance().getBuilder(path_and_uri.second + "/", context->getGlobalContext()->getConfigRef());
        HDFSFSPtr fs = createHDFSFS(builder->get());
...
SinkToStoragePtr StorageHDFS::write(const ASTPtr & query, const StorageMetadataPtr & metadata_snapshot, ContextPtr context_)
{
    ...
    else
    {
        ...
        auto builder = HDFSHandlerFactory::instance().getBuilder(uri_without_path + "/", context_->getGlobalContext()->getConfigRef());
        HDFSFSPtr fs = createHDFSFS(builder->get());
...
void StorageHDFS::truncate(const ASTPtr & /* query */, const StorageMetadataPtr &, ContextPtr local_context, TableExclusiveLockHolder &)
{
    ...
    auto builder = HDFSHandlerFactory::instance().getBuilder(url + "/", local_context->getGlobalContext()->getConfigRef());
    HDFSFSPtr fs = createHDFSFS(builder->get());
